...
bool MySocket::waitForClientConnect()
  {
      ...
      if (clientSocket_ == -1) return false;
      return true;
  }
#endif
...
bool MySocket::initializeServer()
{
  ...
  if (serverSocket_ == -1) return false;

  serverAddress_.sin_family = AF_INET;
  serverAddress_.sin_port = htons(8082);
  serverAddress_.sin_addr.s_addr = INADDR_ANY;

  if (bind(serverSocket_, (struct sockaddr *)&serverAddress_, sizeof(serverAddress_)) == -1) return false;

  if (listen(serverSocket_, 5) == -1) return false;
  return true;
...
bool MySocket::initializeClient()
{
  ...
  if (connect(clientSocket_, (struct sockaddr *)&serverAddress_, sizeof(serverAddress_)) == -1) return false;
  return true;
}