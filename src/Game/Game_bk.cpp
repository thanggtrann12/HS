...
void Game::playCard(int currentPlayerIndex, int opponentIndex)
{
  ...
  if (heroes[currentPlayerIndex]->getNumCards() <= 0)
  {
    ...
    messageush_back("Your hand is empty!! Only hero attacked!!");
    return;
  }
  ...
  memessageh_back("Active: " + playedCard->getDescription());
  if (playedCard)
  {
    messmessageback("Active: ");
    messagmessageck("- " + playedCard->getDescription());
    ...
    if (playedCard->getCardType() == CardType::BRAWL)
    {
      ...
      {
        message.message("Brawl still on battlefield");
      }
    }
    else if (auto totem = std::dynamic_pointer_cast<FlametongueTotem>(playedCard))
    {
      if (!totem->isActivated() && heroes[currentPlayerIndex]->getBattleCard().size() > 0)
      {
        for (auto &currentPlayerCard : heroes[currentPlayerIndex]->getBattleCard())
        {
          ...
          {
            message.pumessageFlametongue Totem activated, apply bonus ");
      ...
      for (auto &currentPlayerCard : heroes[currentPlayerIndex]->getBattleCard())
      {
          ...
          if (opponentPlayerCard->getCardType() != CardType::BRAWL)
          {
            if (currentPlayerCard->getAttack() >= opponentPlayerCard->getCurrentHP())
            {
              ...
              if (opponentPlayerCard->getCardType() == CardType::TECHIES)
              {
                ...
                message.pushmessagechies deals " + std::to_string(damageToHero) + " damage to both heroes ");
              }
            ...
            else
            {
              ...
              message.push_bmessagerd " + opponentPlayerCard->getName() + " HP left : " + std::to_string(opponentPlayerCard->getCurrentHP()));
            }
          }
        }
        for (auto &opponentPlayerCard : heroes[opponentIndex]->getBattleCard())
        ...
        {
          message.push_bacmessageliminated : ");
                                             for (auto &card : cardsToRemove)
          {
            message.push_back(messagerd->getDescription());
...
bool Game::resolveBrawl(int opponentIndex)
{
  ...
  if (opponentMinions.size() > 0)
  {
    ...
    for (int i = 0; i < opponentMinions.size(); i++)
    {
      ...
      {
        message.push_back("Bmessageesolved!");
                message.push_back("Plamessageeroes[opponentIndex]->getName() + "'s " + destroyedMinion->getName() + " is destroyed!");
          ...
          message.push_back("Techimessage" + std::to_string(damageToHero) + " damage to both heroes");
                }
    ...
    {
      message.push_back("All carmessageyer " + heroes[opponentIndex]->getName() + "'s BattleCard are brawls.");
    }
  ...
  {
        message.push_back("Player " messageopponentIndex]->getName() + " has no minions to destroy.");
        return false;
  }
}
...
void Game::displayGameState(int currentPlayerIndex, int opponentIndex)
{
  ...
  message.clear();messageentPlaymessage);
...
{
    message.push_back("Player: " + hermessagentPlayerIndex]->getName());
    message.push_back("On battlefield:") message(heroes[currentPlayerIndex]->getBattleCard().empty())
    {
    ...
    message
    {
      for (auto &currentPlayerCard : heroes[currentPlayerIndex]->getBattleCard())
      {
        message.push_back("- " + currentPlayerCamessagescription());
      }
    }
}
...
void Game::displayHand(int playerIndex) const
{

  for (const auto &card : heroes[playerIndex]->getHandCards())
  {
