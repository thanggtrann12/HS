...
#include <iostream>
GameEntity::GameEntity(std::string n, int a, int h, std::string sk, EntityType tp) : name(n), attack(a), health(h), skill(sk), type(tp) { maxhealth = health; }
...
std::string GameEntity::GetDescription() const
{
  ...
  return name + ": HP[" + std::to_string(health) + "] - ATTACK[" + std::to_string(attack) + "]";
}
void GameEntity::SetAttack(int _newAtk)
{
  attack += _newAtk;
}
bool GameEntity::IsApply()
{
  return isUsed;
}
void GameEntity::Applied()
{
  isUsed = true;
}
